...
pub type ItemListBuilder<C = Rgb888> = WidgetBuilder<ItemListCfg<C>, C>;

/// ItemList configuration: optional custom selected-item style.
pub struct ItemListCfg<C = Rgb888> {
    style_selected: Option<Style<C>>,
}

impl<C> ItemListCfg<C> {
    /// Creates an empty builder.
    pub fn new() -> WidgetBuilder<ItemListCfg<C>, C> {
        WidgetBuilder { common: CommonBuilder::default(), cfg: ItemListCfg { style_selected: None } }
    }
}

impl<C> WidgetBuilder<ItemListCfg<C>, C> {
    ...
    pub fn style_selected(mut self, s: Style<C>) -> Self {
        self.cfg.style_selected = Some(s);
        self
    }
}

impl<C: PixelColor + 'static + From<Rgb888>> WidgetCfg<C> for ItemListCfg<C> {
    fn default_style() -> Style<C> {
        let mut s = Style::default();
        s.bg_color = Some(Rgb888::new(34, 34, 44).into());
        s.border_color = Some(Rgb888::new(70, 70, 90).into());
...
fn default_sel_style<C: From<Rgb888>>() -> Style<C> {
    let mut s = Style::default();
    s.bg_color = Some(Rgb888::new(50, 70, 120).into());
    s
}

impl<C: PixelColor + 'static> super::Widget<C> for ItemListState<C> {
...
pub trait UiItemListExt<C = Rgb888> {
...
impl<C: PixelColor + 'static> UiItemListExt<C> for Ui<C> {
    ...
    fn itemlist_add_item(&mut self, il: ObjRef) -> Option<ObjRef> {
        let (content, sel_style, was_empty) = {
            let s = self.widget::<ItemListState<C>>(il)?;
    ...
    fn itemlist_remove_selected(&mut self, il: ObjRef) -> bool {
        let Some((content, selected)) = self.widget::<ItemListState<C>>(il).map(|s| (s.content, s.selected)) else {
            return false;
        };
        ...
        self.update::<ItemListState<C>, _>(il, |s| s.selected = new_sel);
    ...
    fn itemlist_select(&mut self, il: ObjRef, idx: usize) {
        ...
        if cur != selected {
            self.update::<ItemListState<C>, _>(il, |s| {
                if s.selected != cur {
                    s.selected = cur;
                }
            });
        ...
        self.update::<ItemListState<C>, _>(il, |s| s.selected = nidx);
        ensure_visible(self, il, content, nidx);
        self.send_event(il, crate::event::EventKind::ValueChanged);
    }

    fn itemlist_selected(&self, il: ObjRef) -> usize {
        self.widget::<ItemListState<C>>(il).map(|s| s.selected).unwrap_or(0)
    }

    fn itemlist_len(&self, il: ObjRef) -> usize {
        self.widget::<ItemListState<C>>(il).map(|s| self.children(s.content).len()).unwrap_or(0)
    }
}

/// Scrolls content (translate.y) so the selected item is visible in the viewport (instant, no animation)
fn ensure_visible<C: PixelColor + 'static>(ui: &mut Ui<C>, il: ObjRef, content: ObjRef, selected: usize) {
