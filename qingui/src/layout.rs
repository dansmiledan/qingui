...
pub fn layout_flex<C: embedded_graphics::pixelcolor::PixelColor>(ui: &mut Ui<C>, container: ObjRef, f: &Flex, content: crate::geometry::Rect) {
...
/// `content` is the container's content box in its local coordinate space
/// (origin = padding offsets, size = rect minus padding), computed by Ui.
pub fn layout_grid<C: embedded_graphics::pixelcolor::PixelColor>(ui: &mut Ui<C>, container: ObjRef, g: &Grid, content: crate::geometry::Rect) {
    let area_w = content.w;
    let area_h = content.h;
